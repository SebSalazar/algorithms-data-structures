/*----------------------------------------------------------------------------------------------------------------------------------" 
                                     	COLAS DE PRIORIDAD EN ARREGLO
										  SEBASTIAN SALAZAR - 2017202016
									  	  DANIEL RODRIGUEZ - 20172020009 
"-----------------------------------------------------------------------------------------------------------------------------------*/
#include <iostream>
#include <stdlib.h>
#include <windows.h>
#include <conio.h>

//---Libreria de la cola prioritaria
#include "ColaP.h"
using namespace std;


int main(){
	int opc, num, tam;
	char letra;
	SetConsoleTitle("COLAS DE PRIORIDAD"); 
	system ("COLOR B");
	
	cout<<"\t\t\t\t***************************************************"<<endl;
	cout<<"\t\t\t\t\t\tCOLAS DE PRIORIDAD"<<endl;
	cout<<"\t\t\t\t***************************************************"<<endl;
	cout<<"Ingrese el tamano del arreglo"<<endl;
	cin>>tam;
	cout<<endl;
	ColaP <int>col (tam);
		do{
		system("CLS");
  	    cout<<"\n<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<"<<endl;
        cout<<"\tMENU PRINCIPAL"<<endl;
        cout<<"1> Insertar un numero <nodo> a la cola"<<endl;
        cout<<"2> Atender la Cola"<<endl;
        cout<<"3> Imprimir la Cola "<<endl;
        cout<<"4> Info de los desarrolladores + tamano de la cola"<<endl;
        cout<<"5> Salir"<<endl;
  	    cout<<">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>"<<endl;
        cout<<"\nIngrese Opcion"<<endl;
        cin>>opc;
        while(opc<0 || opc>5){ //---Limitando la entrada de las opciones---
            system("CLS");
            main();
        }
        switch(opc){
        case 1:
        	    system("CLS");
        	    cout<<"INGRESA LA PRIORIDAD"<<endl;
        	    cout<<"Numero = "; cin>>num;
        	    cout<<"INGRESA LA LETRA"<<endl;
        	    cout<<"Letra = "; cin>>letra;
        	    col.insertar(num, letra);     	    
                break;
        case 2:
        	    system("CLS");
        	    cout<<"Atendiendo la Cola: "<<endl;
				cout<<"La prioridad atendida es: "<<col.atender()<<endl;
                break;
        case 3:
        	    system("CLS");
        	    cout<<"Imprimiendo toda la Cola Prioritaria"<<endl;
                col.imprimirColaP();
                break;
        case 4: 
                system("CLS");
                cout<<"\n\n\n\n\n\n\n\n\n\t\t\t\t*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*"<<endl;
			    cout<<"\t\t\t\t\t\tPROGRAMA DESARROLLADO POR: "<<endl;
			    cout<<"\t\t\t\t\tSEBASTIAN CAMILO SALAZAR IBANEZ - 20172020016"<<endl;
			    cout<<"\t\t\t\t\tUNIVERSIDAD DISTRITAL FRANCISCO JOSE DE CALDAS"<<endl;	
			    cout<<"\t\t\t\t\tCIENCIAS DE LA COMPUTACION 1"<<endl;
    			cout<<"\t\t\t\t*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*"<<endl;
    			cout<<"El tamano de la Cola es: "<<col.TamanoCola()<<endl; //El tamaño de la cola es hasta la ultima pocision no nula del arreglo
                break;
        case 5: 
                exit(1);
        }
        //--------Solo para cuando sea una de las opciones-------------
        if(opc<5){
            cout<<"\n\nPresiona cualquier tecla para regresar al menu";
            getch();
            }
        }while(opc!=5);     	
}


